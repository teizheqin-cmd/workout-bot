...
def get_recent_history(limit: int = 999) -> str:
    """Get workouts from the past 30 days"""
    try:
        ws = _get_sheet()
        ...
        recent_rows = [r for r in data_rows if len(r) > 0 and r[0] >= cutoff]

        if not recent_rows:
            # Fallback: last 10 records if none in past 30 days
            recent_rows = data_rows[-10:]
